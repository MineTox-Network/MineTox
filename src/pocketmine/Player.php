			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()){
					...
					$damage = [
						...
						//EntityDamageEvent::MODIFIER_ARMOR => -floor($item->getHpDamage() * $this->inventory->calculateArmorModifier() * 0.04)
					];

					if($target instanceof Player){
						$damage[EntityDamageEvent::MODIFIER_ARMOR] = -floor($item->getHpDamage() * $target->getInventory()->calculateArmorModifier() * 0.04);
					}
